...
**CATEGORIES:**
Assign each direction to one of these types: "UI/UX", "PWA", "Platforms", "Websites", "Web apps".
Also provide 2-3 relevant tags for each.

**GOAL:**
Return ONLY a raw JSON array of 3 objects:
[{ "name": "Direction Name", "type": "Category", "tags": ["tag1", "tag2"] }]
        `.trim();
        ...
        if (!generatedStyles || generatedStyles.length < 3) {
            generatedStyles = [
                { name: "Primary Pigment Gridwork", type: "UI/UX", tags: ["minimal", "bauhaus"] },
                { name: "Tactile Risograph Layering", type: "Websites", tags: ["texture", "retro"] },
                { name: "Kinetic Silhouette Balance", type: "Web apps", tags: ["motion", "organic"] }
            ];
        }
        ...
        setSessions(prev => prev.map(s => {
            ...
            return {
                ...
                artifacts: s.artifacts.map((art, i) => ({
                    ...art,
                    styleName: generatedStyles[i].name,
                    type: generatedStyles[i].type,
                    tags: generatedStyles[i].tags
                }))
            };
        }));

        const generateArtifact = async (artifact: Artifact, styleInfo: any) => {
            try {
                const prompt = `
You are AppStudio™. Create a stunning, high-fidelity UI component for: "${trimmedInput}".

**CONCEPTUAL DIRECTION: ${styleInfo.name}**
**TYPE: ${styleInfo.type}**
**TAGS: ${styleInfo.tags?.join(', ')}**
...
Return ONLY RAW HTML. No markdown fences.
  ...
  return (
    <>
        <div className={`creator-credit ${hasStarted ? 'hide-on-mobile' : ''} flex items-center gap-4`}>
            <span className="opacity-70">Developer: Christo Botha</span>
            <div className="flex items-center gap-3">
                <a href="https://wa.me/2765573133" target="_blank" rel="noreferrer" title="Whatsapp: 065573133" className="hover:text-green-400 transition-colors">
                    <MessageCircleIcon />
                </a>
                <a href="tel:0828451951" title="Telephone: 0828451951" className="hover:text-blue-400 transition-colors">
                    <PhoneIcon />
                </a>
                <a href="mailto:we.enablesystems@gmail.com" title="Email: we.enablesystems@gmail.com" className="hover:text-red-400 transition-colors">
                    <MailIcon />
                </a>
            </div>
        </div>
        ...
        <div className="immersive-app">
            {showPortfolio && <PortfolioViewer onClose={() => setShowPortfolio(false)} />}
            ...
            <div className={`stage-container ${focusedArtifactIndex !== null ? 'mode-focus' : 'mode-split'}`}>
                 <div className={`empty-state ${hasStarted ? 'fade-out' : ''}`}>
                     <div className="empty-content">
                         <h1>AppStudio™</h1>
                         <p>Powered By: 🌐SA-iLabs™</p>
                         <div className="flex gap-4 justify-center mt-4">
                             <button className="surprise-button" onClick={handleSurpriseMe} disabled={isLoading}>
                                 <SparklesIcon /> Surprise Me
                             </button>
                             <button className="surprise-button" onClick={() => setShowPortfolio(true)} disabled={isLoading} style={{ background: 'var(--accent-color)', color: 'white' }}>
                                 View Portfolio
                             </button>
                         </div>
                     </div>
                ...
                {hasStarted && focusedArtifactIndex === null && (
                    <div className="filter-bar">
                        {['All', 'UI/UX', 'PWA', 'Platforms', 'Websites', 'Web apps'].map(filter => (
                            <button 
                                key={filter}
                ...
                {sessions.map((session, sIndex) => {
                    ...
                    const filteredArtifacts = session.artifacts.filter(art => 
                        activeFilter === 'All' || art.type === activeFilter
                    );

                    if (filteredArtifacts.length === 0 && activeFilter !== 'All' && sIndex === currentSessionIndex) {
                        return (
                            <div key={session.id} className={`session-group ${positionClass}`}>
                                <div className="no-results">
                                    No artifacts match the "{activeFilter}" filter in this session.
                                </div>
                            </div>
                        );
                    }
                    ...
                    return (
                        <div key={session.id} className={`session-group ${positionClass}`}>
                            <div className="artifact-grid" ref={sIndex === currentSessionIndex ? gridScrollRef : null}>
                                {filteredArtifacts.map((artifact, aIndex) => {
